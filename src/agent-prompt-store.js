...
- For queued experiments, use \`vr-research-runner <project-dir> run --cwd <code-repo> --command "<cmd>" --metric-regex "<regex>" --seed <id> --git-commit --ask-human\` to claim the move, record a cycle, capture artifacts, and ask for \`continue\` / \`rerun\` / \`synthesize\` / \`brainstorm\` / \`steer\` review. Use \`vr-research-judge <project-dir> --slug <slug> --ask-human\` after meaningful cycles or finish to audit doctor/admit/paper state and surface the next action. Use \`vr-research-runner <project-dir> finish --slug <slug> --aggregate-metric --auto-admit --apply\` to set STATUS, write mean/std frontmatter, apply Queue updates, remove ACTIVE, and log the resolution.

...
test("core building catalog exposes BuildingHub as a manifest-only community source", () => {
  ...
  assert.ok(
    buildingHub.onboarding.variables.some((variable) =>
      variable.setting === "buildingHubAuthProvider" && variable.required === true
    ),
  );
  assert.ok(
    buildingHub.onboarding.steps.some((step) =>
      step.completeWhen?.buildingAccessConfirmed === true
    ),
  );
});

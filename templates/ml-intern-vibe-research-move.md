...
3. Claim the move: create or update the result doc, set `STATUS: active`, add yourself to ACTIVE with agent `0`, remove the move from QUEUE, then commit and push the wiki.
...
8. Resolve or abandon the result doc, write the leaderboard verdict, apply queue updates, update README ACTIVE/LEADERBOARD/QUEUE/LOG, then commit and push the wiki.
...
- Preserve Vibe Research as the research ledger. Your final answer is not enough; the wiki and code repo history are the durable output.

...
- Draw an on-screen arrow at any user-addressable control with `POST $VIBE_RESEARCH_AGENT_TOWN_API/pointer` body `{ target, reason, durationMs, sourceSessionId }`. `target` is one of: `building:<id>` (arrow over a town building, auto-navigates to the Map), `plugin-card:<id>` (points at a BuildingHub card, auto-navigates to BuildingHub), `view:<name>` (points at the sidebar entry for that view), `workspace-tab:<key>` (points at an open workspace tab), `element:<token>` (points at any DOM node tagged `data-agent-target="<token>"`), or a `{x, y}` coord object for raw positioning. The arrow auto-dismisses on click, Esc, timeout, or when a new pointer replaces it. `DELETE /pointer` clears it early. Prefer tokens over coords — the DOM moves; tokens don't.
